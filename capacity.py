...
from utils.utils import salt_pepper_noise, apply_mask, compute_average_ssim, compute_accuracy_ssim
...
def main():
    ...
    avg_accuracy = utils.meters.AverageMeter('Synaptic Utilization', ':.4e')
    ...
    for i, sample in enumerate(tqdm(test_loader)):
        ...
        with torch.no_grad():
            ...
            for i in range(args.num_classes):
                # query_i = image_sequence[0][i].unsqueeze(0).cuda(0)
                query_i = image_sequence[:, i:i + 1, :, :, :].squeeze(1).cuda(0)
                output_i, _, _, _, _ = model(image_sequence.cuda(0), query_i)
                output_i = output_i.view(-1, 1, 28, 28)
                query_collection.append(query_i)
                output_collection.append(output_i)

        query_collection = torch.stack(query_collection, dim=0)
        output_collection = torch.stack(output_collection, dim=0)

        accuracy = compute_accuracy_ssim(query_collection, output_collection, threshold=0.80)
        avg_accuracy.update(accuracy, 1)

    print("平均准确率：", avg_accuracy.avg)
