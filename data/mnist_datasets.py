...
class RepeatMNISTDataset(Dataset):
    def __init__(self, root: str, train: bool, classes: int, dataset_size: int,
                 image_transform: Optional[Callable] = None) -> None:
        self.classes = classes
        self.dataset_size = dataset_size
    ...
    def __getitem__(self, index) -> Tuple:
        ...
        class_data = self.data_by_class[0]
        random_image, random_label, _ = class_data[self.class_indices[0]]
        for i in range(self.classes):
            random_images.append(random_image)
            random_labels.append(random_label)
...
class EMNISTDataset(Dataset):
    def __init__(self, root: str, train: bool, classes: int, dataset_size: int,
        ...
        self.dataset_size = dataset_size
        self.image_transform = image_transform
        self.mnist_data = datasets.EMNIST(root=root, train=train, transform=image_transform,
                                          download=False, split='bymerge')
        # Organize data by class
        self.data_by_class = {}
        for i in range(47):
            self.data_by_class[i] = []
...
class RedundantMNISTDataset(Dataset):
    def __init__(self, root: str, train: bool, classes: int, dataset_size: int,
                 image_transform: Optional[Callable] = None) -> None:
        self.classes = classes
        self.dataset_size = dataset_size
    ...
    def __getitem__(self, index) -> Tuple:
        ...
        random_labels = []
        for i in range(self.classes):
            class_data = self.data_by_class[i % 10]
            random_image, random_label, _ = random.choice(class_data)
            random_images.append(random_image)
            random_labels.append(random_label)
...
if __name__ == '__main__':
    ...
    mnist_dataset = RedundantMNISTDataset(root='/usr/common/datasets/MNIST', train=True, classes=15, dataset_size=6000, image_transform=transform)
