...
class IafPscDelta(NeuronModel):
    ...
    def __call__(self, x_t: torch.Tensor, states: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> Tuple[
        ...
        # z_t: t时刻的神经元输出（0或1），v_t：t时刻的神经元膜电位，r_t：神经元的不应期状态
        z_t, v_t, r_t = states
...
# IF神经元模型
class NonLeakyIafPscDelta(NeuronModel):

    def __init__(self, thr: float = 1.0, perfect_reset: bool = False, refractory_time_steps: int = 0,
                 spike_function: Type[torch.autograd.Function] = SpikeFunction, dampening_factor: float = 1.0) -> None:
        super().__init__()
        self.thr = thr
        self.perfect_reset = perfect_reset
        self.refractory_time_steps = refractory_time_steps
    ...
    def __call__(self, x_t: torch.Tensor, states: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> Tuple[
        ...
        # Integrate membrane.
        v_t, v_scaled = self.integrate(x_t, z_t, v_t, self.thr, self.perfect_reset)
    ...
    @staticmethod
    @torch.jit.script
    def integrate(x_t: torch.Tensor, z_t: torch.Tensor, v_t: torch.Tensor, thr: float,
                  perfect_reset: bool) -> Tuple[torch.Tensor, torch.Tensor]:
        if perfect_reset:
            v_t = v_t * (1.0 - z_t) + x_t
        else:
            v_t = v_t + x_t - z_t * thr
