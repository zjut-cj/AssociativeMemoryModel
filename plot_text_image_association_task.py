...
from data.fashion_mnist_dataset import FashionMNISTDataset
...
from models.network_models import BackUp, AttentionMemoryModel, TextImageAssociation
...
def main():
    parser = argparse.ArgumentParser(description='text image association task')
    ...
    test_set = FashionMNISTDataset(root='/usr/common/datasets/FashionMNIST', train=False, classes=args.num_classes,
    ...
    text_embedding_layer = SpikingTextCNN(IafPscDelta(thr=0.01,
                                                      perfect_reset=args.perfect_reset,
                                                      refractory_time_steps=args.refractory_time_steps,
                                                      tau_mem=args.tau_mem,
                                                      spike_function=SpikeFunction),
    ...
    image_sequence, text, labels, image_query, text_query, targets = None, None, None, None, None, None
    for i, sample in enumerate(test_loader):
        image_sequence, text, labels, image_query, text_query, targets = sample
        break
    ...
    outputs, encoding_outputs, writing_outputs, reading_outputs, decoder_outputs = \
        model(image_sequence, text, text_query)
    outputs_copy = outputs.clone()

    # Get the outputs
    text_encoded = encoding_outputs[0].detach().numpy()
    ...
    mean_rate_text_encoding = np.sum(text_encoded, axis=1) / (1e-3 * args.sequence_length * args.num_time_steps)
    ...
    z_s_enc = np.concatenate((text_encoded[0], images_encoded[0]), axis=1)
    ...
    # Make some plots
    fig, ax = plt.subplots(nrows=1, ncols=image_sequence.size()[1] + 1, sharex='all')
    ...
    ax[-1].set(title='a {}'.format(text_labels[query_label[0]]))
    ...
    ax[0, 0].pcolormesh(text_encoded[0].T, cmap='binary')
    ax[0, 0].set_ylabel('text')
    ax[0, 1].barh(range(args.embedding_size), mean_rate_text_encoding[0])
