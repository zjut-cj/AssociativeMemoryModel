...
from models.network_models import bAbIModel
...
def main_worker(gpu, num_gpus_per_node, args):
    ...
    # Create model
    print("=> creating model '{model_name}'".format(model_name=bAbIModel.__name__))
    model = bAbIModel(
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed and
        ...
        if log_dir and args.logging:
            ...
            log_dir = os.path.join('results', 'babi_task', 'logs', time_stamp +
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (sample, story_length) in enumerate(train_loader):
        ...
        # answer_array = answer.clone().detach().to('cpu').numpy()
        # Compute output
