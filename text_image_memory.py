...
def main_worker(gpu, num_gpus_per_node, args):
    ...
    train_set = FashionMNISTDataset(root=args.dir, train=True, classes=args.num_classes,
                                    dataset_size=3000, image_transform=image_transform)
    test_set = FashionMNISTDataset(root=args.dir, train=False, classes=args.num_classes,
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        image_sequence, text, labels, image_query, text_query, answer = sample
        ...
        if args.gpu is not None:
            ...
            text_query = text_query.cuda(args.gpu, non_blocking=True)
        if torch.cuda.is_available():
            image_target = image_target.cuda(args.gpu, non_blocking=True)
        # Compute output
        output, encoding_outputs, writing_outputs, reading_outputs, decoder_outputs = \
...
def validate(data_loader, model, criterion, args, prefix="Val: "):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(data_loader):
            ...
            # Compute output
            output, *_ = model(image_sequence, text, text_query)
            output = output.view(-1, 1, 28, 28)
            ssim = compute_average_ssim(output, image_target)
            loss = criterion(output, image_target)
